...
<div
	...
	class="size-9 col-span-1 row-span-1 {state.map[x][y].drilled
		? 'border-1 border-black'
		: ''} {state.map[x][y].capped ? 'bg-black! text-black' : ''} {state.map[x][y].leased ||
	state.map[x][y].owned
		? state.map[x][y].oilRemaining < 15
			? 'bg-red-800'
			: state.map[x][y].oilRemaining < 200
				? 'bg-red-600'
				: state.map[x][y].oilRemaining < 500
					? 'bg-yellow-600'
					: state.map[x][y].oilRemaining < 1000
						? 'bg-yellow-300'
						: state.map[x][y].oilRemaining < 1500
							? 'bg-green-400'
							: 'bg-green-900'
		: type == 'OIL_D'
			? 'bg-green-300'
